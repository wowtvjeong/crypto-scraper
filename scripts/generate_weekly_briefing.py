...
3. 목록에 있는 기사들 중에서 실제로 방송에서 다룰 소재는 각 섹션당 딱 2~3개씩,
   전체 합쳐서 총 6~9개 소재만 골라서 다룰 것. 나머지는 과감히 생략한다.
   (25건을 다 언급하는 게 아니라, 그 중 이번 주를 대표할 만한 핵심 소재만 추려내는 것이 목표)
   비슷한 내용의 기사가 여러 개면 그것들을 하나의 소재로 묶어서 한 번만 언급할 것
4. 마크다운 기호(*, #, - 등)나 불릿포인트는 절대 쓰지 말 것 — 그대로 소리내어 읽을 문장으로만 작성
5. 전체 분량은 800~1200자 내외 (5~8분 분량의 1인 브리핑 방송 기준)
...
def build_source_list(articles: list) -> str:
    lines = []
    for a in articles:
        reason = (a.get("reason") or "")[:40]  # 토큰 절약을 위해 이유는 40자까지만
...
def main():
    ...
    # 실제로 대본에서 다룰 소재는 6~9개뿐이라, 후보군은 상위 20건이면 충분함
